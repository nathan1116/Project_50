...
class MainApp(QMainWindow, ui):
    ...
    def data(self):
        ...
        try:
            sql = "select Opername,pihao,IndexNo,Barcode_1,Barcode_2,InjuryDes,Result,jianyanyuan,Time,Note from Report where pihao = '"+str(pihao)+"' and Opername= '返工超声检测' order by IndexNo"
    ...
    def tongji(self):
        ...
        sql0 = "select COUNT(*) from Report where pihao = '"+str(pihao)+"' and Opername= '返工超声检测'"
        cur.execute(sql0)
        zs = cur.fetchall()
        for z in zs:
            z = list(z)
        ...
        sql1 = "select COUNT(*) from Report where pihao = '"+str(pihao)+"' and Opername= '返工超声检测' and Result = '合格'"
        cur.execute(sql1)
        hs = cur.fetchall()
        for h in hs:
            h = list(h)
        ...
        sql2 = "select COUNT(*) from Report where pihao = '"+str(pihao)+"' and Opername= '返工超声检测' and Result = '不合格'"
        cur.execute(sql2)
        bs = cur.fetchall()
        for b in bs:
            b = list(b)
        ...
        sql21 = "select COUNT(*) from Report where pihao = '"+str(pihao)+"' and Opername= '返工超声检测' and Result = '不合格' and InjuryDes like '%纵%'"
        cur.execute(sql21)
        bas = cur.fetchall()
        for ba in bas:
            ba = list(ba)
        ...
        sql22 = "select COUNT(*) from Report where pihao = '"+str(pihao)+"' and Opername= '返工超声检测' and Result = '不合格' and InjuryDes like '%草%' and InjuryDes like '%mm横%'"
        cur.execute(sql22)
        bbs = cur.fetchall()
        for bb in bbs:
            bb = list(bb)
        ...
        sql23 = "select COUNT(*) from Report where pihao = '"+str(pihao)+"' and Opername= '返工超声检测' and Result = '不合格' and InjuryDes like '%尺寸%'"
        cur.execute(sql23)
        bcs = cur.fetchall()
        for bc in bcs:
            bc = list(bc)
        ...
        sql0 = "select COUNT(*) from Report where pihao = '"+str(pihao)+"' and Opername= '返工超声检测' and Time like '%"+str(Time)+"%'"
        cur.execute(sql0)
        zs = cur.fetchall()
        for z in zs:
            z = list(z)
        ...
        sql1 = "select COUNT(*) from Report where pihao = '"+str(pihao)+"' and Opername= '返工超声检测' and Result = '合格' and Time like '%"+str(Time)+"%'"
        cur.execute(sql1)
        hs = cur.fetchall()
        for h in hs:
            h = list(h)
        ...
        sql2 = "select COUNT(*) from Report where pihao = '"+str(pihao)+"' and Opername= '返工超声检测' and Result = '不合格' and Time like '%"+str(Time)+"%'"
        cur.execute(sql2)
        bs = cur.fetchall()
        for b in bs:
            b = list(b)
        ...
        sql21 = "select COUNT(*) from Report where pihao = '"+str(pihao)+"' and Opername= '返工超声检测' and Result = '不合格' and InjuryDes like '%纵%' and Time like '%"+str(Time)+"%'"
        cur.execute(sql21)
        bas = cur.fetchall()
        for ba in bas:
            ba = list(ba)
        ...
        sql22 = "select COUNT(*) from Report where pihao = '"+str(pihao)+"' and Opername= '返工超声检测' and Result = '不合格' and InjuryDes like '%草%' and  InjuryDes like '%mm横%' and Time like '%"+str(Time)+"%'"
        cur.execute(sql22)
        bbs = cur.fetchall()
        for bb in bbs:
            bb = list(bb)
        ...
        sql23 = "select COUNT(*) from Report where pihao = '"+str(pihao)+"' and Opername= '返工超声检测' and Result = '不合格' and InjuryDes like '%尺寸%' and Time like '%"+str(Time)+"%'"
        cur.execute(sql23)
        bcs = cur.fetchall()
        for bc in bcs:
            bc = list(bc)
    ...
    def Barcode_Scan(self):
        ...
        if num != 0:
            ...
            sql = "select count(*) from Report where (Result='待定' and Barcode_1='"+str(Barcode)+"' and Opername='初始超声检测') or (Result='待定' and Barcode_2='"+str(Barcode)+"' and Opername='初始超声检测' )"
            cur.execute(sql)
            dums = cur.fetchall()
            for dum in dums:
                dum = list(dum)
            ...
            if dum != 0:
                conn50 = pymysql.connect(host='192.168.100.2',database='50',user='root',password='123456')
                cur = conn50.cursor()
                sql = "select count(*) from Report where (Opername='返工超声检测' and Barcode_1='"+str(Barcode)+"') or (Opername='返工超声检测' and Barcode_2='"+str(Barcode)+"')"
                cur.execute(sql)
                lums = cur.fetchall()
                for lum in lums:
                    lum = list(lum)
                ...
                if lum == 0:
                    ...
                    QMessageBox.information(None, '错误', '管号重复或已完成超声返工检测！')
                    self.Scan.clear()
            else:
   ...
   #     pihao = self.pihao.text()
    ...
    def tijiao_hege(self):
        ...
        if a != '':
            try:
                ...
                cur.execute("select max(IndexNo) from Report where pihao = '"+str(pihao)+"' and Opername= '返工超声检测'")
                ...
                sql = "insert into Report (Opername,pihao,IndexNo,Barcode_1,Barcode_2,InjuryDes,Result,jianyanyuan,Time,Note) values ('返工超声检测','"+str(pihao)+"','"+str(IndexNo)+"','"+str(Barcode_1)+"','"+str(Barcode_2)+"','无超标缺陷 尺寸合格','合格','"+str(jianyanyuan)+"','"+str(Time)+"','"+str(biaozhun)+"')"
    ...
    def tijiao_buhege(self):
        ...
        if a != '' or shangqing != '':
            try:
                ...
                sql = "insert into Report (Opername,pihao,IndexNo,Barcode_1,Barcode_2,InjuryDes,Result,jianyanyuan,Time,Note) values ('返工超声检测','"+str(pihao)+"','"+str(IndexNo)+"','"+str(Barcode_1)+"','"+str(Barcode_2)+"','"+str(shangqing)+"','不合格','"+str(jianyanyuan)+"','"+str(Time)+"','"+str(biaozhun)+"')"
    ...
    def youjian(self,pos):
        ...
        if action == item1:
            ...
            sql = "delete from Report where pihao = '"+str(pihao)+"' and IndexNo = '"+str(dele)+"' and Opername= '返工超声检测' "
            cur.execute(sql)
            conn50.commit()
            conn50.close()
            self.data()
        ...
        if action == item2:
            ...
            sql = "update Report set  Result = '合格',InjuryDes='无超标缺陷 尺寸合格' where pihao = '"+str(pihao)+"' and IndexNo = '"+str(upd)+"' and Opername= '返工超声检测'"
            cur.execute(sql)
            conn50.commit()
            conn50.close()
            self.data()
        ...
        if action == item3:
            ...
            upd = table_data[row_num][2]            
            upd = str(int(upd)).zfill(3)           
            sql = "update Report set  Result = '不合格',InjuryDes='"+str(shangqing)+"' where pihao = '"+str(pihao)+"' and IndexNo = '"+str(upd)+"' and Opername= '返工超声检测'"
            cur.execute(sql)
            conn50.commit()
            conn50.close()
            self.shangqing.clear()
    ...
    def jilu(self):
        ...
        conn50 = pymysql.connect(host='192.168.100.2',database='50',user='root',password='123456')
        cur = conn50.cursor()
        sql = "select IndexNo,InjuryDes,Result,jianyanyuan,Time,Barcode_1,Barcode_2  from Report where pihao = '"+str(pihao)+"' and Opername= '返工超声检测' order by IndexNo"
        cur.execute(sql)
        ...
        wb.save("C:/Users/Administrator/Desktop/检验记录/返工超声记录"+str(pihao)+".xlsx")

        conn50.close()


        wb = openpyxl.load_workbook("C:/Users/Administrator/Desktop/检验记录/返工超声记录"+str(pihao)+".xlsx")
        ws = wb['Sheet1']
        for i in range(100):
            n = 30*i + 35
...
def main():
    ...
    splash = QSplashScreen(QPixmap("2.png"))
